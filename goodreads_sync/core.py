...
class HttpHelper(object):
    ...
    def __init__(self, gui=None, plugin_action=None):
        ...
        if proxy:
            # Under macOS, for some reason, httplib2.socks doesn't get properly loaded.
            # This ugly hack just checks if socks *was* loaded or not, and sets
            # proxy_type either to the correct setting or to the constant 4.
            # Note that httplib2.ProxyInfo() *requires* proxy_type to be set, or else
    ...
    def create_oauth_client(self, user_name=None, oauth_token=None, oauth_secret=None):
        consumer = oauth.Consumer(key=self.devkey_token, secret=self.devkey_secret)
    ...
    def _oauth_request_post(
        self, oauth_client, url, body="", success_status="200", method="POST"
    ):
    ...
    def get_request_token_secret(self):
        ...
        request_token = {
            key_value[0].decode("utf-8"): key_value[1].decode("utf-8")
            for key_value in request_token
        }
    ...
    def get_goodreads_user_id(self, oauth_token, oauth_secret):
        ...
        response, content = self._oauth_request_get(
            oauth_client, "%s/api/auth_user" % cfg.URL_HTTPS
        )
        if not response:
            return None
        root = et.fromstring(content)
    ...
    def create_shelf(
        self, user_name, new_shelf_name, is_featured, is_exclusive, is_sortable
    ):
        # Creates a shelf
        ...
        response, content = self._oauth_request_post(
            oauth_client, url, body, success_status="201"
        )
        if response:
            return True
        else:
    ...
    def get_shelf_list(self, user_id):
        ...
        while True:
            ...
            page = page + 1
            url = "%s/shelf/list.xml?user_id=%s&page=%d" % (
                cfg.URL_HTTPS,
                user_id,
                page,
            )
    ...
    def add_remove_book_to_shelf(
        self, oauth_client, shelf_name, goodreads_id, action="add"
    ):
        ...
        _response, content = self._oauth_request_post(
            oauth_client, url, body, success_status
        )
        if _response:
            if action == "add":
    ...
    def create_review(
        self, oauth_client, shelf_name, goodreads_id, rating, date_read, review_text
    ):
    ...
    def update_review(
        ...
        success_status = "200"
        body = urlencode(body_info)
        response, _content = self._oauth_request_post(
            oauth_client, url, body, success_status, method="PUT"
        )
        if response:
            return True
        else:
    ...
    def update_status(
        ...
        debug_print(
            "HttpHelper::update_status: reading_progress=%s, comment=%s"
            % (reading_progress, comment)
        )
    ...
    def get_statuses(
        ...
        # Return True if status was updated, False if not.
        debug_print(
            "HttpHelper::get_statuses: reading_progress=%s, comment=%s"
            % (reading_progress, comment)
        )
        if not reading_progress and not comment:
            return True
    ...
    def get_goodreads_id_for_isbn(self, isbn):
        ...
        if (
            ...
            or content == "No book with that ISBN"
        ):
            return ""
        return content
    ...
    def get_goodreads_books_on_shelves(self, user_name, shelves, per_page=100):
        ...
        debug_print(
            "HttpHelper::get_goodreads_books_on_shelves: user_name=%s" % (user_name,)
        )
        oauth_client = self.create_oauth_client(user_name)
        ...
        for shelf in shelves:
            ...
            self.plugin_action.progressbar_label(
                _("Syncing from shelf: {0}").format(shelf_name)
            )
            page = 0
            while True:
                ...
                page = page + 1
                debug_print(
                    "HttpHelper::get_goodreads_books_on_shelves: shelf='%s', page=%s"
                    % (shelf_name, page)
                )
    ...
    def search_for_goodreads_books(self, title="", authors=""):
        ...
        for work_node in work_nodes:
            ...
            if book["goodreads_author"] == "NOT A BOOK":
                ...
                continue
            (title, series) = self._convert_goodreads_title_with_series(
                work_node.findtext("best_book/title").strip()
            )
    ...
    def _convert_review_xml_node_to_book(self, review_node, include_work=False):
        ...
        (title, series) = self._convert_goodreads_title_with_series(
            book_node.findtext("title").strip()
        )
        book["goodreads_title"] = title
        book["goodreads_series"] = series
        ...
        if shelve_nodes is not None and len(shelve_nodes):
            shelves = [
                shelve_node.attrib.get("name").strip() for shelve_node in shelve_nodes
            ]
            ...
            book["goodreads_started_at"] = self._parse_goodreads_date(
                review_node.findtext("started_at")
            )
            book["goodreads_read_at"] = self._parse_goodreads_date(
                review_node.findtext("read_at")
            )
            book["goodreads_date_added"] = self._parse_goodreads_date(
                review_node.findtext("date_added")
            )
            book["goodreads_date_updated"] = self._parse_goodreads_date(
                review_node.findtext("date_updated")
            )
    ...
    def get_xml_tree(self, content):
        ...
        if root is None:
            ...
            f.close()
            raise ValueError(
                "The shelf contains a corrupting response from Goodreads. "
                ...
                + "The failed xml can be found at:<br>"
                + cpath
            )
        return root
    ...
    def get_edition_books_for_work_id(self, work_id):
        ...
        edition_nodes = root.xpath(
            '//div[@class="leftContainer workEditions"]/div[@class="elementList clearFix"]'
        )
        for edition_node in edition_nodes:
            ...
            for idx, data_row_node in enumerate(
                edition_data_node.xpath('div[@class="dataRow"]')
            ):
                if idx == 0:
                    continue  # Will be the title
                text = tostring(
                    data_row_node, method="text", encoding="unicode"
                ).strip()
            ...
            goodreads_edition_book["goodreads_id"] = re.search(
                r"/book/show/(\d+)", book_url
            ).groups(0)[0]
            goodreads_edition_book["goodreads_title"] = "".join(
                edition_data_node.xpath(
                    'div[@class="dataRow"]/a[@class="bookTitle"]/text()'
                )
            )
            cover_url = "".join(
                edition_node.xpath('div[@class="leftAlignedImage"]/a/img/@src')
            )
            ...
            goodreads_edition_book["goodreads_isbn"] = ""
            isbn_node = edition_data_node.xpath(
                'div[@class="moreDetails hideDetails"]/div[@class="dataRow"][2]/div[@class="dataValue"]/span[@class="greyText"]/text()'
            )
            if len(isbn_node) > 0:
                isbn = None
...
class IdCaches(object):
    ...
    def build_caches(self):
        ...
        cb_cache = {}
        db = self.gui.library_view.model().db
        calibre_ids = db.data.search_getting_ids(
            "identifiers:goodreads:True", search_restriction=""
        )
        for calibre_id in calibre_ids:
            goodreads_id = db.get_identifiers(calibre_id, index_is_id=True).get(
                "goodreads", ""
            )
            if goodreads_id:
...
class CalibreSearcher(object):
    ...
    fuzzy_author_patterns = [
        (re.compile(pat, re.IGNORECASE), repl)
        for pat, repl in [(r'[()\'";,|]', ""), (r"\.", ". "), (r"\s+", " ")]
    ]
        ...
        try:
            ...
            for calibre_id in calibre_book_ids:
                ...
                if len(fuzzy_match_title) == 0:
                    ...
                    found_fuzzy_title = self.fuzzyit(
                        search_book["calibre_title"], self.fuzzy_title_patterns
                    )
    ...
    def get_calibre_data_for_book(self, book, calibre_id):
        ...
        book["calibre_title_sort"] = (
            book["calibre_title"] if mi.title_sort is None else mi.title_sort
        )
    ...
    def get_uploadable_columns(self, db, mi, book):
        ...
        reading_progress_column = cfg.plugin_prefs[cfg.STORE_PLUGIN].get(
            cfg.KEY_READING_PROGRESS_COLUMN, ""
        )
        book["calibre_reading_progress"] = -1
    ...
    def build_cache(self):
        ...
        for bid in db.data.iterallids():
            ...
            if reverse_author != authors[0]:
                fuzzy_author_ln_fn = self.fuzzyit(
                    reverse_author, self.fuzzy_author_patterns
                )
...
class CalibreDbHelper(object):
    def apply_actions_to_calibre(self, gui, goodreads_books, actions):
        ...
        for sync_action in actions:
            if sync_action["column"] == "tags":
                self._apply_tag_changes_to_books(
                    goodreads_books, sync_action["action"], sync_action["value"]
                )
            elif sync_action["column"] == "rating":
                self._apply_rating_changes_to_books(
                    goodreads_books, sync_action["action"], sync_action["value"]
                )
            elif sync_action["column"] == "comments":
                self._apply_comment_changes_to_books(
                    goodreads_books, sync_action["action"], sync_action["value"]
                )
            else:
                # Applying values to a custom column
                self._apply_custom_column_changes_to_books(
                    goodreads_books,
                    sync_action["column"],
                    sync_action["action"],
                    sync_action["value"],
    ...
    def _apply_custom_column_changes_to_books(
        self, goodreads_books, column, action, value, special_action
    ):
        """
        ...
        if column not in self.custom_columns:
            ...
            return error_dialog(
                ...
                "You have a sync rule for custom column '%s' which does not exist in this library.<p>"
                % column
        ...
        if typ == "bool":
            for book in goodreads_books:
                ...
                if action == "REMOVE":
                    existing_value = self.db.get_custom(
                        calibre_id, label=label, index_is_id=True
                    )
            ...
            for book in goodreads_books:
                ...
                if action == "ADD":
                    debug_print(
                        "_apply_custom_column_changes_to_books: rating - value=%s, existing_value=%s"
            ...
            raise ValueError(
                "Unsupported custom column type for: " + column + " of " + typ
            )
