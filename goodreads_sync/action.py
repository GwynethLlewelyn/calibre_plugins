...
from calibre_plugins.goodreads_sync.common_icons import (
    set_plugin_icon_resources,
    get_icon,
)
from calibre_plugins.goodreads_sync.common_menus import (
...
from calibre_plugins.goodreads_sync.core import (
    CalibreSearcher,
    HttpHelper,
    IdCaches,
    update_calibre_isbn_if_required,
)
from calibre_plugins.goodreads_sync.dialogs import (
    DoAddRemoveDialog,
    DoShelfSyncDialog,
    SwitchEditionDialog,
    PickGoodreadsBookDialog,
    ActionStatus,
    ChooseShelvesToSyncDialog,
    UpdateReadingProgressDialog,
)

PLUGIN_ICONS = [
    "images/goodreads_sync.png",
    "images/refresh.png",
    "images/shelf.png",
    "images/shelf_exclusive.png",
    "images/add_to_shelf.png",
    "images/remove_from_shelf.png",
    "images/sync_from_shelf.png",
    "images/sync_from_shelf_lg.png",
    "images/add_to_shelf_lg.png",
    "images/remove_from_shelf_lg.png",
    "images/view_book.png",
    "images/view_shelf.png",
    "images/edit_shelf_add_action.png",
    "images/edit_shelf_add_action_lg.png",
    "images/edit_sync_action.png",
    "images/edit_sync_action_lg.png",
    "images/link.png",
    "images/link_add.png",
    "images/link_add_lg.png",
    "images/link_delete.png",
    "images/tag_maps_lg.png",
    "images/authorise.png",
    "images/tags_download.png",
    "images/tags_upload.png",
    "images/rating_add.png",
    "images/rating_sync.png",
    "images/dateread_add.png",
    "images/dateread_sync.png",
    "images/rating_dateread_add.png",
    "images/rating_dateread_sync.png",
    "images/review_add.png",
    "images/review_sync.png",
]
...
class GoodreadsSyncAction(InterfaceAction):
    ...
    def rebuild_menus(self):
        ...
        if len(self.users) > 0:
            ...
            if c.get(cfg.KEY_DISPLAY_ADD, True):
                self.create_action_with_users_sub_menu(
                    m, _("Add to shelf") + "...", "add", "imases/add_to_shelf.png"
                )
            ...
            if c.get(cfg.KEY_DISPLAY_VIEW_SHELF, True):
                m.addSeparator()
                self.create_sub_menu_for_users_action(
                    m, _("View shelf"), "images/view_shelf.png"
                )
            m.addSeparator()
            ...
            m.addSeparator()
            self.linked_book_submenu = m.addMenu(
                get_icon("images/link.png"), _("Linked book")
            )

            create_menu_action_unique(
                self,
                self.linked_book_submenu,
                _("View linked book"),
                "images/view_book.png",
                _("Open a web browser page showing the linked Goodreads book"),
                triggered=self.view_linked_books,
            )
            create_menu_action_unique(
                self,
                self.linked_book_submenu,
                _("Switch Goodreads Edition") + "...",
                "images/link_add.png",
                _("Link to a different edition of a Goodreads book"),
            ...
            create_menu_action_unique(
                ...
                _("Remove link"),
                "images/link_delete.png",
                _("Clear the link with Goodreads"),
        ...
        create_menu_action_unique(
            self, m, _("&Help"), "help.png", shortcut=False, triggered=cfg.show_help
        )
    ...
    def create_action_with_users_sub_menu(self, parent_menu, title, action, image_name):
        ...
        if len(list(self.users.keys())) > 1:
            ...
            for user_name in sorted(self.users.keys()):
                user_sub_menu = sub_menu.addMenu(
                    get_icon("user_profile.png"), _("User: {0}").format(user_name)
                )
                user_sub_menu.setStatusTip(user_sub_menu.title())
                unique_name = 'User "%s" %s' % (user_name, title)
                if action in ["add", "remove"]:
                    triggered_action = partial(
                        self.add_or_remove_to_shelf, action, user_name
                    )
                elif action in ["progress"]:
                    triggered_action = partial(
                        self.update_reading_progress, action, user_name
                    )
                else:
    ...
    def create_sub_menu_for_shelves_action(self, parent_menu, user_name, title):
        ...
        if shelves:
            for shelf in shelves:
                ...
                image_name = (
                    "images/shelf_exclusive.png" if is_exclusive else "images/shelf.png"
                )
                if active:
                    ...
                    ac.triggered.connect(
                        partial(self.grhttp.view_shelf, user_name, shelf_name)
                    )
    ...
    def create_shelves_tags_menu_item(self, parent_menu):
        ...
        if len(self.users) > 1:
            ...
            for user_name in sorted(self.users.keys()):
                self.create_shelves_tags_action(
                    sub_menu,
                    user_name,
                    _("User: {0}").format(user_name),
                    "user_profile.png",
                    _("Download shelves your book is on as tags"),
                    is_download=True,
                    unique_name='User "%s" Download tags from shelves' % (user_name,),
                )
            sub_menu = parent_menu.addMenu(
                get_icon("images/tags_upload.png"), _("Upload tags as shelves")
            )
            ...
            for user_name in sorted(self.users.keys()):
                self.create_shelves_tags_action(
                    ...
                    _("User: {0}").format(user_name),
                    "user_profile.png",
                    _("Add book to shelves represented by your tags"),
                    is_download=False,
            ...
            self.create_shelves_tags_action(
                parent_menu,
                user_name,
                _("Upload tags as shelves"),
                "images/tags_upload.png",
                _("Add book to shelves represented by your tags"),
                is_download=False,
            )

    def create_shelves_tags_action(
        self, parent, user_name, title, image, tooltip, is_download, unique_name=""
    ):
        if not unique_name:
            unique_name = 'User "%s" %s' % (user_name, title)
        if is_download:
    ...
    def add_or_remove_to_shelf(self, action, user_name):
        ...
        d = DoAddRemoveDialog(
            self.gui, self.grhttp, self.id_caches, user_name, action, calibre_books
        )
        d.exec_()
    ...
    def sync_shelves(self, user_name):
        ...
        if len(shelves) == 0:
            return error_dialog(
                ...
                _("Unable to Sync"),
                _(
                    "You must specify sync actions or columns for at least one shelf first."
                ),
                show=True,
            )

        choose_dialog = ChooseShelvesToSyncDialog(
            self.gui, self, self.grhttp, user_name, shelves
        )
        choose_dialog.exec_()
        ...
        if choose_dialog.goodreads_shelf_books is None:
            return error_dialog(
                self.gui,
                _("Unable to Sync"),
                _("Unable to retrieve books for selected shelves."),
                show=True,
            )
        ...
        d = DoShelfSyncDialog(
            ...
            self.grhttp,
            user_name,
            choose_dialog.selected_shelves,
            choose_dialog.goodreads_shelf_books,
            self.calibre_searcher,
        )
        d.exec_()
        if d.result() == d.Accepted:
            ...
            updated_ids = [
                book["calibre_id"] for book in d.goodreads_books if book["updated"]
            ]
    ...
    def _get_shelves_valid_for_sync(self, user_name):
        ...
        rating_column = cfg.plugin_prefs[cfg.STORE_PLUGIN].get(
            cfg.KEY_RATING_COLUMN, None
        )
        date_read_column = cfg.plugin_prefs[cfg.STORE_PLUGIN].get(
            cfg.KEY_DATE_READ_COLUMN, None
        )
        review_text_column = cfg.plugin_prefs[cfg.STORE_PLUGIN].get(
            cfg.KEY_REVIEW_TEXT_COLUMN, None
        )
        sync_shelves = []
    ...
    def _update_calibre_database_ids_after_sync(self, goodreads_books):
        ...
        for book in goodreads_books:
            ...
            book["updated"] = (
                book["status"] in [ActionStatus.ADD_EMPTY, ActionStatus.VALID]
    ...
    def search_to_link_books(self, calibre_books):
        update_isbn = cfg.plugin_prefs[cfg.STORE_PLUGIN].get(
            cfg.KEY_UPDATE_ISBN, "NEVER"
        )
        updated_ids = []
        for row, calibre_book in enumerate(calibre_books):
            ...
            if isbn:
                ...
                if goodreads_id_for_isbn:
                    goodreads_book = self.grhttp.get_goodreads_book_for_id(
                        goodreads_id_for_isbn
                    )
            ...
            is_isbn_match = (
                goodreads_id_for_isbn is not None and len(goodreads_id_for_isbn) > 0
            )
            d = PickGoodreadsBookDialog(
                self.gui,
                self.grhttp,
                self.id_caches,
                calibre_book,
            ...
            if update_isbn == "ALWAYS" or missing_isbn:
                ...
                if goodreads_isbn is not None:
                    update_calibre_isbn_if_required(
                        calibre_book, goodreads_isbn, update_isbn
                    )
    ...
    def _update_calibre_database_ids_for_selection(self, calibre_books):
        ...
        for book in calibre_books:
            ...
            if goodreads_id == orig_goodreads_id:
                continue
            db.set_identifier(
                calibre_id, "goodreads", goodreads_id, notify=False, commit=False
            )
            book["updated"] = True
        db.commit()
    ...
    def remove_links(self):
        ...
        if not question_dialog(
            ...
            "<p>"
            + _(
                "Removing any linked Goodreads ids may result in you having to manually "
                "select the link again for these book(s)."
            )
            + "<p>"
            + _("Are you sure you want to remove the link(s)?"),
        ...
        try:
            ...
            self.gui.status_bar.showMessage(
                _("Removing Goodreads ids from books") + "..."
            )
            for row in rows:
                calibre_id = db.id(row.row())
        ...
        for row in rows:
            ...
            if not goodreads_id:
                error_dialog(
                    ...
                    _("No Goodreads Id"),
                    _(
                        "You cannot switch editions for a book that is not already linked to Goodreads.<br>"
                    ),
                    show=True,
                )
                return
    ...
    def switch_edition_for_linked_books(self, calibre_books):
        ...
        for row, calibre_book in enumerate(calibre_books):
            ...
            d = SwitchEditionDialog(
                self.gui, self.id_caches, calibre_book, edition_books, next_book
            )
            d.exec_()
            if d.result() != d.Accepted:
                break
    ...
    def download_tags(self, user_name, is_download):
        ...
        tag_column = cfg.plugin_prefs[cfg.STORE_PLUGIN].get(
            cfg.KEY_TAG_MAPPING_COLUMN, "tags"
        )
        is_multiple = True
        db = self.gui.library_view.model().db
        ...
        for row in rows:
            calibre_id = db.id(row.row())
            goodreads_id = self.id_caches.calibre_to_goodreads_ids().get(
                calibre_id, None
            )
            if not goodreads_id:
                continue
            ...
            if not book:
                continue
            orig_calibre_tags = self._get_calibre_tags_for_book(
                db, calibre_id, tag_column, tag_column_label, is_multiple
            )
            # For a custom column we will always overwrite with fresh values
            # For the tags column we will always append
            calibre_tags = (
                set(list(orig_calibre_tags)) if tag_column == "tags" else set()
            )
            ...
            if calibre_tags != orig_calibre_tags:
                if tag_column == "tags":
                    db.set_tags(
                        calibre_id, list(calibre_tags), append=True, commit=False
                    )
                else:
                    if is_multiple:
    ...
    def upload_tags(self, user_name, is_download):
        ...
        for row in rows:
            ...
            if book:
                ...
                current_shelves = []
            calibre_tags = self._get_calibre_tags_for_book(
                db, calibre_id, tag_column, tag_column_label, is_multiple
            )
            for calibre_tag in calibre_tags:
                for shelf_name, mapped_tag_values in tag_mappings.items():
                    ...
                    self.grhttp.add_remove_book_to_shelf(
                        oauth_client, shelf_name, goodreads_id
                    )
                    shelve_additions += 1
    ...
    def _get_calibre_tags_for_book(
        self, db, calibre_id, tag_column, tag_column_label, is_multiple
    ):
        if tag_column == "tags":
    ...
    def progressbar(
        self, window_title=_("Goodreads Sync progress"), on_top=False, show=False
    ):
        self.pb = ProgressBarDialog(
            parent=self.gui, window_title=window_title, on_top=on_top
        )
        if show:
            self.pb.show()
