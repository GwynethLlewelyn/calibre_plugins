...
from calibre_plugins.goodreads_sync.common_dialogs import (
    SizePersistedDialog,
    KeyboardConfigDialog,
)
from calibre_plugins.goodreads_sync.common_widgets import (
    NoWheelComboBox,
...
CUSTOM_COLUMN_DEFAULTS = {
    SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_TAGS: {
        ...
        "config_tool_tip": _(
            'For use with the "Download tags from shelves" and "Upload tags as shelves" menu items'
        ),
        "initial_items": ["tags"],
    },
    SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_RATING: {
        ...
        "config_tool_tip": _(
            'For use with the "Add to shelf" and "Sync from shelf" menu items\nto synchronise with your Goodreads review for a book'
        ),
        "initial_items": [""],
    },
    ...
    if schema_version < 1.6:
        ...
        for user_info in users_settings.values():
            ...
            if KEY_TAG_MAPPING_COLUMN in user_info.keys():
                tag_mapping_column = user_info.get(
                    KEY_TAG_MAPPING_COLUMN, tag_mapping_column
                )
...
class ISBNComboBox(QComboBox):
    VALUES = [
        ("NEVER", _("Never modify the calibre ISBN")),
...
class DateTimeColumnComboBox(NoWheelComboBox):
    VALUE_ADD_MAP = OrderedDict([("none", _("None")), ("today", _("Today"))])
...
class AddShelfDialog(QDialog):
    def __init__(self, parent=None, shelves=[]):
        ...
        for key, row, col, tooltip in [
            (
                "Exclusive",
                1,
                0,
                _("A book can only be on one of your exclusive shelves"),
            ),
            (
                "Sortable",
                1,
                1,
                _("Allowing customizing the order of books on this shelf"),
            ),
            (
                "Featured",
                1,
                2,
                _("One shelf only can be featured at the top of your profile"),
            ),
        ]:
...
class MaintainActionsTableWidget(QTableWidget):
        ...
        for sync_action in sync_actions:
            ...
            if column_key == "tags" or column_key in self.custom_columns:
                ...
                info_dialog(
                    ...
                    _("Invalid column"),
                    _(
                        "Removing the column '{0}' action as the column cannot be found"
                    ).format(column_key),
                    show=True,
                )
    ...
    def populate_table_row(self, row, sync_action):
        ...
        combo = CustomColumnComboBox(
            self, self.custom_columns, column_key, initial_items=["tags"]
        )
        ...
        if self.is_bool_custom_column(column_key):
            ...
            self.setCellWidget(
                row,
                2,
                DateTimeColumnComboBox(
                    self, sync_action["value"], is_add_values=self.is_shelf_add_actions
                ),
            )
        elif self.is_enumeration_custom_column(column_key):
            values = self.custom_columns[column_key]["display"]["enum_values"]
            self.setCellWidget(
                row, 2, EnumColumnComboBox(self, sync_action["value"], values)
            )
        elif column_key == "tags":
            self.setCellWidget(
                row, 2, self.create_tags_edit(sync_action["value"], self.all_tags)
            )
        elif self.custom_columns[column_key]["is_multiple"] is not None:
            values = self.get_taglike_column_values(column_key)
            self.setCellWidget(
                row, 2, self.create_tags_edit(sync_action["value"], values)
            )
        else:
    ...
    def column_type_changed(self, combo, row):
        ...
        if self.is_bool_custom_column(column_key):
            ...
            self.setCellWidget(
                row,
                2,
                DateTimeColumnComboBox(self, is_add_values=self.is_shelf_add_actions),
            )
        elif self.is_enumeration_custom_column(column_key):
            values = self.custom_columns[column_key]["display"]["enum_values"]
    ...
    def is_bool_custom_column(self, column_key):
        return (
            ...
            and self.custom_columns[column_key]["datatype"] == "bool"
        )

    def is_datetime_custom_column(self, column_key):
        return (
            column_key.startswith("#")
            and self.custom_columns[column_key]["datatype"] == "datetime"
        )
    ...
    def delete_rows(self):
        ...
        if len(selrows) > 1:
            message = _(
                "Are you sure you want to delete the selected {0} actions?"
            ).format(len(selrows))
        if not question_dialog(
            self, _("Are you sure?"), "<p>" + message, show_copy_button=False
        ):
            return
    ...
    def get_data(self):
        ...
        for row in range(self.rowCount()):
            ...
            column_key = sync_action["column"] = self.cellWidget(
                row, 1
            ).get_selected_column()
            if self.is_bool_custom_column(
                column_key
            ) or self.is_enumeration_custom_column(column_key):
                value = unicode(self.cellWidget(row, 2).currentText()).strip()
            elif self.is_datetime_custom_column(column_key):
                value = self.cellWidget(row, 2).selected_key()
            elif (
                column_key == "tags"
                or self.custom_columns[column_key]["is_multiple"] is not None
            ):
...
class MaintainActionsDialog(SizePersistedDialog):
    ...
    def __init__(self, parent, is_shelf_add_actions, shelves, custom_columns, all_tags):
        SizePersistedDialog.__init__(
            self, parent, "goodreads sync plugin:maintain sync action dialog"
        )
        ...
        if is_shelf_add_actions:
            ...
            if len(shelves) == 1:
                title = _("Edit add to shelf actions for '{0}' shelf").format(
                    shelves[0]["name"]
                )
            message = _(
                "&What should calibre do with matching linked books when you click 'Add to shelf'?"
            )
            title_layout = ImageTitleLayout(
                self, "images/edit_shelf_add_action_lg.png", title
            )
        else:
            self.setWindowTitle(_("Edit Shelf Sync Actions"))
            title = _("Edit sync actions for {0} shelves").format(len(shelves))
            if len(shelves) == 1:
                title = _("Edit sync actions for '{0}' shelf").format(
                    shelves[0]["name"]
                )
            message = _(
                "&What should calibre do with matching linked books when you click 'Sync from shelf'?"
            )
            title_layout = ImageTitleLayout(
                self, "images/edit_sync_action_lg.png", title
            )
        ...
        self.sync_actions_table = MaintainActionsTableWidget(
            self, is_shelf_add_actions, custom_columns, all_tags
        )
        ...
        if is_shelf_add_actions:
            ...
            date_read_title = _(
                "Upload date read to Goodreads when adding to this shelf"
            )
            review_text_title = _(
                "Upload review text to Goodreads when adding to this shelf"
            )
            upload_rating_enabled = (
                shelves[0].get(KEY_ADD_RATING, False) and not is_currently_reading_shelf
            )
            upload_date_read_enabled = (
                shelves[0].get(KEY_ADD_DATE_READ, False)
                and not is_currently_reading_shelf
            )
            upload_review_text_enabled = (
                shelves[0].get(KEY_ADD_REVIEW_TEXT, False)
                and not is_currently_reading_shelf
            )
        else:
            rating_title = _("Sync rating from Goodreads when syncing from this shelf")
            date_read_title = _(
                "Sync date read from Goodreads when syncing from this shelf"
            )
            review_text_title = _(
                "Sync review text from Goodreads when syncing from this shelf"
            )
            upload_rating_enabled = (
                shelves[0].get(KEY_SYNC_RATING, False)
                and not is_currently_reading_shelf
            )
            upload_date_read_enabled = (
                shelves[0].get(KEY_SYNC_DATE_READ, False)
                and not is_currently_reading_shelf
            )
            upload_review_text_enabled = (
                shelves[0].get(KEY_SYNC_REVIEW_TEXT, False)
                and not is_currently_reading_shelf
            )
...
class ShelvesTableWidget(QTableWidget):
    ...
    def create_context_menu(self):
        ...
        self.edit_shelf_add_action = QAction(
            get_icon("images/edit_shelf_add_action.png"),
            _("Edit shelf add actions") + "...",
            self,
        )
        self.edit_shelf_add_action.triggered.connect(self.edit_shelf_add_actions)
        self.addAction(self.edit_shelf_add_action)
        self.edit_sync_action = QAction(
            get_icon("images/edit_sync_action.png"),
            _("Edit sync actions") + "...",
            self,
        )
        ...
        self.addAction(sep2)
        self.edit_shelves_action = QAction(
            get_icon("images/view_book.png"), _("Edit Shelves on Goodreads"), self
        )
    ...
    def populate_table_row(self, row, shelf):
        ...
        self.setCellWidget(
            row, 3, self._create_tags_edit(", ".join(shelf.get(KEY_TAG_MAPPINGS, [])))
        )
    ...
    def _build_actions_text(self, shelf, add):
        ...
        if add:
            ...
            if shelf.get(KEY_SYNC_RATING, False) and shelf.get(
                KEY_SYNC_DATE_READ, False
            ):
        ...
        if actions_key not in shelf:
            ...
            for sync_action in shelf[actions_key]:
                ...
                if sync_action_type == "ADD":
                    message = message + _("Add '{0}' to column '{1}', ").format(
                        sync_action["value"], sync_action["column"]
                    )
                elif sync_action_type == "REMOVE":
                    message = message + _("Remove '{0}' from column '{1}', ").format(
                        sync_action["value"], sync_action["column"]
                    )
        if len(message) > 2:
            message = message[:-2]
    ...
    def edit_actions(self, is_shelf_add_actions):
        ...
        d = MaintainActionsDialog(
            self,
            is_shelf_add_actions,
            selected_shelves,
            self.custom_columns,
            self.all_tags,
        )
...
class ConfigWidget(QWidget):
    def __init__(self, plugin_action, grhttp):
        ...
        action_custom_columns = self.get_custom_columns(
            ["bool", "text", "comments", "datetime", "enumeration"]
        )
        self.all_tags = self.plugin_action.gui.library_view.model().db.all_tags()
        self._shelves_table = ShelvesTableWidget(
            self, action_custom_columns, self.all_tags
        )
        ...
        self._sync_shelves_button.setToolTip(
            _("Refresh the list of shelves and book counts from Goodreads")
        )
        ...
        self._edit_add_to_shelf_action_button = QPushButton(
            _("Edit 'Shelf Add' Actions") + "...", self
        )
        self._edit_add_to_shelf_action_button.setToolTip(
            _("Edit actions to apply when using 'Add to shelf'")
        )
        self._edit_add_to_shelf_action_button.setIcon(
            get_icon("images/edit_shelf_add_action.png")
        )
        self._edit_add_to_shelf_action_button.clicked.connect(
            self._shelves_table.edit_shelf_add_actions
        )
        user_buttons_layout.addWidget(self._edit_add_to_shelf_action_button)
        self._edit_sync_action_button = QPushButton(
            _("Edit 'Sync' Actions") + "...", self
        )
        self._edit_sync_action_button.setToolTip(
            _("Edit actions to apply when using 'Sync from shelf'")
        )
        self._edit_sync_action_button.setIcon(get_icon("images/edit_sync_action.png"))
        self._edit_sync_action_button.clicked.connect(
            self._shelves_table.edit_shelf_sync_actions
        )
        ...
        self._isbn_combo = ISBNComboBox(
            self, other_options.get(KEY_UPDATE_ISBN, "NEVER")
        )
        ...
        for row, key, display, tooltip in [
            ...
            (
                2,
                KEY_DISPLAY_REMOVE,
                _("Display the 'Remove from shelf' menu option"),
                "",
            ),
            (
                3,
                KEY_DISPLAY_UPDATE_PROGRESS,
                _("Display the 'Update reading progress' menu option"),
                "",
            ),
            ...
            (
                6,
                KEY_PROGRESS_IS_PERCENT,
                _("Reading progress is percent read"),
                _(
                    "If the reading progress is the percent read, check this. Otherwise, the reading progress the page number."
                ),
            ),
        ...
        self.sync_custom_columns[SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_TAGS] = {
            "current_columns": self.get_text_custom_columns
        }
        self.sync_custom_columns[SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_RATING] = {
            "current_columns": self.get_rating_custom_columns
        }
        self.sync_custom_columns[SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_DATE_READ] = {
            "current_columns": self.get_date_custom_columns
        }
        self.sync_custom_columns[SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_REVIEW_TEXT] = {
            "current_columns": self.get_long_text_custom_columns
        }
        self.sync_custom_columns[SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_READING_PROGRESS] = {
            "current_columns": self.get_number_custom_columns
        }

        self._tag_column_combo = self.create_custom_column_controls(
            columns_group_box_layout2, SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_TAGS
        )
        self._tag_column_combo.currentIndexChanged[int].connect(
            self.tag_column_combo_changed
        )
        self._rating_column_combo = self.create_custom_column_controls(
            columns_group_box_layout2, SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_RATING
        )
        self._dateread_column_combo = self.create_custom_column_controls(
            columns_group_box_layout2, SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_DATE_READ
        )
        self._review_text_column_combo = self.create_custom_column_controls(
            columns_group_box_layout2, SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_REVIEW_TEXT
        )
        self._reading_progress_column_combo = self.create_custom_column_controls(
        ...
        self._tag_column_combo.populate_combo(
            ...
            initial_items=CUSTOM_COLUMN_DEFAULTS[
                SYNC_CUSTOM_COLUMN_DEFAULT_LOOKUP_TAGS
            ]["initial_items"],
        )
        ...
        keyboard_shortcuts_button = QPushButton(
            " " + _("Keyboard shortcuts") + "... ", self
        )
    ...
    def save_settings(self):
        ...
        other_options = plugin_prefs[STORE_PLUGIN]
        other_options[KEY_TAG_MAPPING_COLUMN] = (
            self._tag_column_combo.get_selected_column()
        )
        other_options[KEY_RATING_COLUMN] = (
            self._rating_column_combo.get_selected_column()
        )
        other_options[KEY_READING_PROGRESS_COLUMN] = (
            self._reading_progress_column_combo.get_selected_column()
        )
        other_options[KEY_DATE_READ_COLUMN] = (
            self._dateread_column_combo.get_selected_column()
        )
        other_options[KEY_REVIEW_TEXT_COLUMN] = (
            self._review_text_column_combo.get_selected_column()
        )
        for key in [
            KEY_DISPLAY_ADD,
            KEY_DISPLAY_REMOVE,
    ...
    def get_custom_columns(self, column_types):
        if self.supports_create_custom_column:
            custom_columns = (
                self.get_create_new_custom_column_instance.current_columns()
            )
        else:
    ...
    def add_user_profile(self):
        ...
        new_user_name, ok = QInputDialog.getText(
            ...
            _("Add new user"),
            _(
                "You must create a profile for each Goodreads user you will connect to.<br>Enter a unique display name for this user profile:"
            ),
            text="Default",
        )
        if not ok:
    ...
    def delete_user_profile(self):
        ...
        if not question_dialog(
            ...
            _("Are you sure?"),
            "<p>"
            + _("Do you want to delete the user profile for '{0}'").format(
                self.user_name
            ),
    ...
    def authorize_plugin(self):
        ...
        (user_token, user_secret) = self.grhttp.get_user_token_secret(
            oauth_token=request_token, oauth_secret=request_secret
        )
        if not user_token:
            return
        ...
        user_id = self.grhttp.get_goodreads_user_id(
            oauth_token=user_token, oauth_secret=user_secret
        )
        if not user_id:
            return error_dialog(
                self,
                _("Goodreads failure"),
                _("Unable to obtain the user id"),
                show=True,
            )
            ...
            for old_shelf_info in old_shelves:
                for new_shelf_info in new_shelves:
                    if new_shelf_info["name"] == old_shelf_info["name"]:
                        ...
                        new_shelf_info[KEY_ADD_ACTIONS] = old_shelf_info.get(
                            KEY_ADD_ACTIONS, []
                        )
                        new_shelf_info[KEY_ADD_RATING] = old_shelf_info.get(
                            KEY_ADD_RATING, False
                        )
                        new_shelf_info[KEY_ADD_DATE_READ] = old_shelf_info.get(
                            KEY_ADD_DATE_READ, False
                        )
                        new_shelf_info[KEY_SYNC_ACTIONS] = old_shelf_info.get(
                            KEY_SYNC_ACTIONS, []
                        )
                        new_shelf_info[KEY_SYNC_RATING] = old_shelf_info.get(
                            KEY_SYNC_RATING, False
                        )
                        new_shelf_info[KEY_SYNC_DATE_READ] = old_shelf_info.get(
                            KEY_SYNC_DATE_READ, False
                        )
                        new_shelf_info[KEY_SYNC_REVIEW_TEXT] = old_shelf_info.get(
                            KEY_SYNC_REVIEW_TEXT, False
                        )
                        new_shelf_info[KEY_TAG_MAPPINGS] = old_shelf_info.get(
                            KEY_TAG_MAPPINGS, []
                        )
                        break
    ...
    def add_shelf(self):
        ...
        if new_shelf_name in shelves:
            return error_dialog(
                self,
                _("Duplicate shelf name"),
                _("There is already a shelf named:") + " " + new_shelf_name,
                show=True,
            )
        # Perform the update with Goodreads
        if self.grhttp.create_shelf(
            self.user_name, new_shelf_name, d.is_featured, d.is_exclusive, d.is_sortable
        ):
    ...
    def create_custom_column_controls(
        self, columns_group_box_layout, custom_col_name, min_width=120
    ):
        ...
        custom_column_combo = CustomColumnComboBox(
            ...
            create_column_callback=create_column_callback,
        )
